...
class If_Else_AST:
    ...
    def code(self):
        ...
        return self.conditon.false_code(l1) + \
...
class Bool_Not_AST:
    ...
    def indented(self, level):
        op = { '<':'>=', '=':'!=', '>':'<=',
               '<=':'>', '!=':'=', '>=':'<' }        
        return indent(op[self.op], level) + \
               self.left.indented(level+1) + \
               self.right.indented(level+1)
    def false_code(self, label):
        op = { '<':'if_icmplt', '=':'if_icmpeq', '>':'if_icmpgt',
...
class Bool_And_Or_AST:
    ...
    def true_code(self, label):
        ...
        if self.op == 'or':
            ...
            nxt_lbl = label_generator.next()
            return self.left.false_code(nxt_lbl) + \
                   self.right.false_code(nxt_lbl) + \
                   'goto ' + label + '\n' + nxt_lbl + \
                   ':' + '\n'
    ...
    def false_code(self, label):
        ...
        if self.op == 'or':
            ...
            return self.left.true_code(l2) + \
                   self.right.false_code(label) + l2 + ':' + \
                   '\n'
        else:
...
def boolean_expression():
    ...
    while scanner.lookahead() == Token.OR:
        ...
        tree = boolean_term()
        result = Bool_And_Or_AST(result, operator[op], tree)
    return result
...
def boolean_factor():
    not_count = 0
    while scanner.lookahead() == Token.NOT:
        scanner.consume(Token.NOT)
        not_count += 1
    condition = comparison()
    if not_count % 2 == 1:
        return Bool_Not_AST(condition)
    else:
        return condition 
